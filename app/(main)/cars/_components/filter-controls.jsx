...
const CarFilterControls = ({filters,currentFilters,onFilterChange,onClearFilter}) => {
  ...
  const {make,bodyType,fuelType,transmission,priceRange,color}=currentFilters;
  ...
  return (
    <div className='space-y-6'>
       <div className='space-y-4'>
         ...
         <div className='px-2'>
           <Slider min={filters.priceRange.min} max={filters.priceRange.max} step={100} value={priceRange} onValueChange={(value)=>onFilterChange('priceRange',value)} />
         </div>
        ...
        {filterSelection.map((section)=>(
           <div key={section.id} className='space-y-3'>
              <h4 className='text-sm font-medium flex justify-between items-center'>
                <span>{section.title}</span>
                {section.currentValue && (
                   <button className='text-xs text-gray-600 flex items-center cursor-pointer' onClick={()=>onClearFilter(section.id)}>
              ...
              <div className='flex flex-wrap gap-2 max-h-60 overflow-y-auto pr-1 custom-scrollbar' >
                {section.options.map(option=>(
                  <Badge key={option.value} variant={section.currentValue===option.value?'default':'outline'} className={`cursor-pointer px-3 py-1 ${section.currentValue ===option.value ? 'bg-blue-100 hover:bg-blue-200 text-blue-900 border-blue-200':'bg-white hover:bg-gray-100 text-gray-700'}`}
                    onClick={()=>{section.onChange(section.currentValue===option.value ? '':option.value)}}
                  >
                    {option.label}
