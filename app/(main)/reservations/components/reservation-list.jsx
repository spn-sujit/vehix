...
const ReservationsList = ({ initialData }) => {
    ...
    const {loading:cancelling,fn:cancelBookingFn,error:cancelError}= useFetch(cancelTestDrives);
       ...
       {upcomingBookings.length ===0 ?(
        ...
        <div className="space-y-3">
         {upcomingBookings.map((booking)=>(
          <TestDriveCard key={booking.id} booking={booking} onCancel={handleCancelBooking}  isCancelling={cancelling} showActions/>
         ))} 
        </div>
       )}
     </div>
         ...
         <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {pastBookings.map((booking)=>(
              <TestDriveCard key={booking.id} booking={booking} showActions={false} isPast/>
            ))}
          </div>
         </div>
       )}
